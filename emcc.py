...
def parse_args(newargs):
  ...
  for i in range(len(newargs)):
    ...
    # Record USE_PTHREADS setting because it controls whether --shared-memory is passed to lld
      ...
      # Ignore the next argument which rather than trying to parse it.
      # This is needed because llvm args could, for example, start with `-o` and
      # we don't won't to confure that with a normal `-o` flag.
      skip = True
